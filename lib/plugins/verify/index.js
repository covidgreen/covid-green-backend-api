      ...
      if (platform === 'test') {
        ...
        try {
          ca = pki.createCaStore([
            pki.certificateFromPem(deviceVerification.safetyNetRootCa)
          ])
        } catch (err) {
          throwError(200, 'CA missing or invalid')
        }

        if (deviceVerification.validateCertChain) {
          if (
        ...
        if (data.nonce !== nonce) {
          this.log.info(
            { expected: data.nonce, actual: nonce },
            '103: Nonce does not match expected value'
          )
        ...
        if (
          ...
          this.log.info(
            { expected: deviceVerification, actual: data.apkPackageName },
            '104: Package name does not match expected value'
          )
