...
const key = S.string().format('uuid').required()
...
const verify = {
  body: S.object()
    .prop('selfIsolationEndDate', S.string().format(S.FORMATS.DATE).required())
...
const send = {
  body: S.object()
    ...
    .prop('senderEmail', S.string().format(S.FORMATS.EMAIL).required())
    .prop(
      'recipients',
      S.array()
