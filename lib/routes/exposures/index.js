...
async function exposures(server, options, done) {
  if (options.routes.disableExposure) {
    server.log.info('Exposure Endpoint: Off')
    done()
    return
  }

  server.log.info('Exposure Endpoint: On')
  const hash = (value) => {
  ...
  const exchangeCodeForToken = async (id, code) => {
    ...
    const { rows } = await server.pg.write.query(
      tokenInsert({ id, onsetDate, testType, testMethod: testMethod || 'pcr' })
    )
    const [{ id: token }] = rows
    ...
    return { onsetDate, testType, testMethod: testMethod || 'pcr', token }
  }
  ...
  server.route({
    ...
    handler: async (request, response) => {
      ...
      if (options.verifyProxy.url !== '') {
        ...
        const { onsetDate, testType, testMethod, token } =
          await exchangeCodeForToken(id, codeHash)
  ...
  server.route({
    ...
    handler: async (request, response) => {
      ...
      if (options.verifyProxy.url !== '') {
        ...
        if (!result.error && !chaffRequest(request.headers)) {
          await server.pg.write.query(
            metricsInsert({
  ...
  const uploadHandler = async (request) => {
    ...
    if (chaffRequest(request.headers)) {
      ...
      if (token) {
        ...
        const [{ createdAt, onsetDate, testMethod }] = rows
        ...
        resolvedTestMethod = testMethod || 'pcr'
      } else {
  ...
  server.route({
    ...
    handler: async (request) => {
      ...
      // if returning data is disabled then just return emopty array
      if (options.exposures.disableExposureData) {
        more = false
      }
