...
async function exposures(server, options, done) {
  ...
  const exchangeCodeForToken = async (id, code) => {
    ...
    const [
      { verificationId, lastUpdatedAt, onsetDate, testType, sendCount }
    ] = hashRows

    if (
      differenceInMinutes(new Date(), new Date(lastUpdatedAt)) >
  ...
  server.route({
    ...
    handler: async (request, response) => {
      ...
      if (options.verifyProxy.url !== '') {
        const result = await proxyRequest(response, '/api/verify', { code })
