...
const exposureInsert = ({ exposures, regions, testType, testMethod }) => {
  const query = SQL`INSERT INTO exposures (key_data, rolling_period, rolling_start_number, transmission_risk_level, regions, test_type, days_since_onset, test_method) VALUES `
...
const tokenUpdate = ({ id, token }) =>
  SQL`UPDATE upload_tokens
      ...
      RETURNING created_at AS "createdAt", onset_date AS "onsetDate", test_type AS "testType", test_method AS "testMethod"`

const tokenInsert = ({ id, onsetDate, testType, testMethod }) =>
  SQL`INSERT INTO upload_tokens (reg_id, onset_date, test_type, test_method)
      VALUES (${id}, ${onsetDate}, ${testType}, ${testMethod})
      RETURNING id`
