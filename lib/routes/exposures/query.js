...
const exposureInsert = ({ exposures, regions }) => {
  ...
  for (const [
    index,
    { keyData, rollingPeriod, rollingStartNumber, transmissionRiskLevel }
...
const tokenDelete = ({ id, token }) =>
  SQL`DELETE FROM upload_tokens
      ...
      RETURNING created_at AS "createdAt", onset_date AS "onsetDate"`
