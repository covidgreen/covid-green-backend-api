  ...
  server.route({
    ...
    handler: async request => {
      ...
      try {
        ...
        if ((!refreshToken || refreshToken === 'null') && options.security.allowNoToken) {
          const refresh = (await randomBytes(32)).toString('hex')
        ...
        return {
          token: server.jwt.sign(
            ...
            {
              audience: options.security.jwtIssuer,
              expiresIn: temporary ? `${options.security.tokenLifetimeNoRefresh}` : `${options.security.tokenLifetime}m`,
              issuer: options.security.jwtIssuer
            }
          )
        }
