...
async function register(server, options, done) {
  ...
  if (!options.routes.disableRegister) {
    server.log.info('Register endpoint: On')
    server.route({
      method: 'POST',
      url: '/register',
    ...
    server.log.info('Register endpoint: off')
  }
  ...
  server.route({
    ...
    handler: async (request) => {
      const { os, version } = request.body || {}
      const refreshToken = request.headers.authorization
        ? request.headers.authorization.replace(/^Bearer/, '').trim()
        : ''

      try {
        let resolvedId, temporary

        if (
          (!refreshToken || refreshToken === 'null') &&
          options.security.allowNoToken
        ) {
          const refresh = (await randomBytes(32)).toString('hex')
          ...
          /* request.log.info(
            { id },
            'registration created for missing refresh token'
          ) */
        } else {
          let data

          try {
            data = server.jwt.verify(refreshToken)
          ...
          try {
            if (decrypt(encryptedRefresh) !== data.refresh) {
              throw new Error()
            }
          } catch {
          ...
          await server.pg.write.query(refreshUpdate({ id: resolvedId }))
        }
        ...
        return {
          token: server.jwt.sign(
            { id: resolvedId },
            {
              audience: options.security.jwtIssuer,
              expiresIn: temporary
                ? `${options.security.tokenLifetimeNoRefresh}`
                : `${options.security.tokenLifetime}m`,
              issuer: options.security.jwtIssuer
            }
          )
        }
      } catch (err) {
        request.log.info({ refreshToken }, 'token renewal failed')
