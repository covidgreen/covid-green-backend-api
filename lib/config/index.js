...
async function getConfig() {
  ...
  if (isProduction) {
    ...
    config.security.codeLifetimeDeeplink = Number(
      await getParameter('security_code_lifetime_deeplink_mins')
    )
    ...
    config.deeplinks.packageName = await getParameter(
      'deeplink_android_package_name'
    )
    config.deeplinks.appstoreLink = await getParameter('deeplink_appstore_link')
    config.deeplinks.defaultWebPage = await getParameter(
