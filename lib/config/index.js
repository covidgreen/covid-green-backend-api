...
async function getConfig() {
  ...
  if (isProduction) {
    ...
    config.security.allowNoToken = /true/i.test(await getOptionalParameter(
      'security_allow_no_token',
      false
    ))
    ...
    if (config.routes.qr) {
      ...
      config.qr.queueUrl = await getParameter('qr_queue_url')
