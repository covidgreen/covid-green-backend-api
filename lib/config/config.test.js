...
describe('configuration', () => {
  ...
  it('returns values according to environment variables', async () => {
    ...
    const NOTICES_RATE_LIMIT_SECS = faker.random.number()
    ...
    const DEEP_LINKS_ALLOWED = faker.random.boolean()
    const LOG_CALLBACK_REQUEST = faker.random.boolean()
    const ALLOW_NO_TOKEN = faker.random.boolean()
    const CODE_LIFETIME_DEEPLINK_MINS = faker.random.number()
